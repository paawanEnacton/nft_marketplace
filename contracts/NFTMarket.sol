...
contract NFTMarket is ERC721URIStorage, Ownable {
    ...
    // if the price is not 0 => an NFT was listed
    // if price is 0 && tokenURI is empty string => NFT was transferred (either bought, or the listing was cancelled)
    event NFTTransfer(
        uint256 tokenID,
        address from,
        address to,
    ...
    function cancelListing(uint256 tokenID) public {
        ...
        require(
            ...
            "NFTMarket: you're not the owner"
        );
        ERC721(address(this)).transferFrom(address(this), msg.sender, tokenID);
        clearListing(tokenID);
        emit NFTTransfer(tokenID, address(this), msg.sender, "", 0);
